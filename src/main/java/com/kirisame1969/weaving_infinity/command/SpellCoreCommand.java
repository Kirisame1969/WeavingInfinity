...
public class SpellCoreCommand {
    ...
    private static int addModule(CommandContext<CommandSourceStack> context, String moduleId) {
        ...
        try {
            ...
            // 检查模块是否存在
            if (!ModuleRegistry.hasModule(rl)) {
                source.sendFailure(Component.translatable("command.spellcore.module_not_found"));
                return 0;
            }

            // 尝试添加模块到法术核心
            if (SpellCoreItem.addModule(heldItem, rl)) {
                final String finalModuleId = moduleId; // 创建一个 effectively final 变量供 lambda 表达式使用
                source.sendSuccess(() -> Component.translatable("command.spellcore.module_added", finalModuleId), true);
                return 1;
            } else {
                source.sendFailure(Component.translatable("command.spellcore.module_slots_full"));
                return 0;
            }
    ...
    private static int listModules(CommandContext<CommandSourceStack> context) {
        ...
        try {
            ...
            // 获取模块列表并显示
            var modules = SpellCoreItem.getModuleIds(heldItem);
            if (modules.isEmpty()) {
                ...
                for (int i = 0; i < modules.size(); i++) {
                    ...
                    source.sendSuccess(() -> Component.literal((index + 1) + ". " + modules.get(index)), false);
                }
            }
